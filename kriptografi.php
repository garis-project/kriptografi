...
class kriptografi{	
	...
	function tampilArray2D($array){
		for($b = 0; $b < count($array[0]); $b++){		
		}
	...
	function keyexpansion($key, $Nb){ //kunci & banyak blok
		...
		static $rcon = array(
			...
			array(0x14,0x00,0x00,0x00),
			array(0x28,0x00,0x00,0x00),
			array(0x50,0x00,0x00,0x00),
			array(0x1B,0x00,0x00,0x00),
			array(0x36,0x00,0x00,0x00)
		);
		
		$this->tampilArray2D($rcon);
		# Salin Key ke variable w (Key Schedule)
		# ord() : untuk mendapatkan nilai ASCII dari suatu karakter
		for($i=0; $i<$Nb; $i++){
			for($j=0; $j<4; $j++) {
				$w[$i][$j] = ord($arrkey[$count]);
				$count++;
			}
		}
		$this->tampilArray2D($w);
		...
		for( $i=4; $i<44; $i++){
			for($j=0; $j<4; $j++) {
				...
				if ($i%4==0) {					
					...
					$w[$j][$i] = $this->subword($wmin1) ^ $wmin4 ^ $rcon[$count_rcon][$j];
	...
	# State XOR Key Schedule
	function AddRoundKey($data, $pos_w, $w, $Nb) {		
		$state = array();
		for($i=0; $i<4; $i++){
			//$temp_w = $pos_w;
			for($j=0; $j < $Nb; $j++) {
				$state[$i][$j] = $data[$i][$j] ^ $w[$i][$pos_w+$j];
			}
		}	
		return $state;		
	}
	...
	function encrypt($input, $w, $Nb, $Nr){
		#Menghitung jumlah blok plainteks
		$op=count($input);
		$help=$op;
		$sisa 	 =$op%16;
		$tambah=16-$sisa;
		...
		for($z=0; $z < $NbPlain; $z++){
			...
			$this->tampilArray2D($state);
			for($i=0; $i<4; $i++){
				for($j=0; $j < $Nb; $j++) {
					$stateIjeOld = $state[$i][$j];
					$state[$i][$j] = $state[$i][$j] ^ $w[$i][$pos_w + $j];
				}
			}
			$this->tampilArray2D($state);
			$pos_w = $pos_w + $Nb;			
			for ($i=0; $i<$Nr-1; $i++) {
				$state = $this->SubByte($state, $Nb);
				$this->tampilArray2D($state);
				$state = $this->ShiftRow($state, $Nb);
				$this->tampilArray2D($state);
				$state = $this->MixColumns($state, $Nb);
				$this->tampilArray2D($state);
				$state = $this->AddRoundKey($state, $pos_w, $w, $Nb);
				$this->tampilArray2D($state);
				$pos_w = $pos_w + $Nb;
			}	
			$state = $this->SubByte($state, $Nb);
			...
			$state = $this->AddRoundKey($state, $pos_w, $w, $Nb);
			$this->tampilArray2D($state);
			foreach($state as $state_){
				foreach($state_ as $data1) {
					$cipher[$idP] = ($data1);
					$idP++;
				}
			}
	...
	function InvMixColumns($data, $Nb){
		$state = array();
	...
	function decrypt($input, $w, $Nb, $Nr){
			//$data = str_split($input);
			$data = ($input);
			$NbPlain = count($data)/16;
			$plain = array(); $idC=0;
			for($z=0; $z < $NbPlain ; $z++){
				$state = array();
				$count 	= 0;
				...
				for($i=0; $i<4; $i++){
					$x = 0;
					for($j= $pos_w; $j < (($Nr+1)*4); $j++) {
						$y = $x++;		
						$state[$i][$y] = $state[$i][$y] ^ $w[$i][$j];
					}
				}				
				$this->tampilArray2D($state);
				for ($i=0; $i<$Nr-1; $i++) {
					$state = $this->InvShiftRow($state, $Nb);				
					$this->tampilArray2D($state);
					$state = $this->InvSubByte($state, $Nb);
					$this->tampilArray2D($state);
					$pos_w = $pos_w - $Nb;
					$state = $this->AddRoundKey($state, $pos_w, $w, $Nb);
					$this->tampilArray2D($state);
					$state = $this->InvMixColumns($state, $Nb);			
					$this->tampilArray2D($state);
				}
				$state = $this->InvShiftRow($state, $Nb);
				$this->tampilArray2D($state);
	...
	function EnkripCaesar($data, $kunci){
	$array_pl = array();
		for($i=0; $i < strlen($data); $i++){
			$array_pl[$i] =(ord($data[$i])+strlen($kunci))%127;
		}
		return $array_pl;
	}

	function CekKunci($kunci){
		# Blok Data AES
		# Menghitung Panjang Key 
		# Panjang Key AES (16 bytes)
		# untuk jenis AES (AES-128 bit)
		$Nb=4;
		$Nk=strlen($kunci);
		if ($Nk != 16){
			for($i=0; $i < (16- $Nk); $i++){
	...
	function HasilEnkripsi($cipherteks){
		$hasil="";
		for($i=0; $i< count($cipherteks); $i++){
			$hasil.=sprintf("%02s",dechex($cipherteks[$i]));
		}
		return $hasil;
	}

	function HasilDeskripsi($result,$key){
		$ukur=count($result);
		$out="";
		for($i=0; $i<$ukur;$i++){
			$x=($result[$i]-strlen($key))%127;
			if($x<0){
				($x+=127);
			}
			$out.=chr($x);
		...
		$i=0;
		$find=0;
		while($array[$i]==chr(32-strlen($key))){
			$find+=1;
			$i++;
		}
		$output=strrev(substr($new, $find));
		return $output;
	}
	...
	function convertToDecimal($data){
		...
		$decimal=array();
		for ($i=0;$i<$length;$i++){
			$hexa=substr($data,$i*2,2);
			$decimal[$i]=hexdec($hexa);
		}
		return $decimal;
	}

	function aturKunci($kunci){
		$kunciB = $this->CekKunci($kunci);
		...
		$key = $this->keyexpansion($kunciB, $Nb); 
		$bar = count($key);
		$col = count($key[0]);
		$this->tampilArray2D($key);
		return $key;
	}

	function enkripsi($plainteks,$kunci){
		$cipherAes=$this->EnkripCaesar($plainteks,$kunci);
		# Validasi untuk memfilter panjang key
		$key=$this->aturKunci($kunci);
		$Nk = 16;
		$Nb = 4;
		$Nr = ($Nk/4) + $Nb + 2;
		// $lama1 = microtime(true);
		$cipherteks =$this->encrypt($cipherAes, $key, $Nb, $Nr);
		$decode=$this->HasilEnkripsi($cipherteks);
		return $decode;
	}
	function deskripsi($cipherteks,$kunci){
		$decimal = $this->convertToDecimal($cipherteks);
		$key=$this->aturKunci($kunci);
		$Nk = 16;
		$Nb = 4;
		$Nr = ($Nk/4) + $Nb + 2;
		$result = $this->decrypt($decimal, $key, $Nb, $Nr);
		$plainteks=$this->HasilDeskripsi($result,$kunci);
		return $plainteks;
	}
}